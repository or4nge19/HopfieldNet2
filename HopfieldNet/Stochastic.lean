...
instance decidableEqState :
  ...
  intro s₁ s₂
  apply decidable_of_iff (∀ u, s₁.act u = s₂.act u) ⟨fun h ↦ State.ext h, fun h u ↦ by rw [h]⟩
...
@[simp]
lemma gibbsUpdate_possible_states
   (v : U)
...
lemma gibbsUpdate_prob_negative
  ...
  let local_field := s.net wθ u
  let Z := ENNReal.ofReal (Real.exp (local_field / T)) + ENNReal.ofReal (Real.exp (-local_field / T))
  (NN.State.gibbsUpdateSingleNeuron wθ s T u) (NN.State.updateNeuron s u (-1) (by exact Or.inr rfl)) =
    ENNReal.ofReal (Real.exp (-local_field / T)) / Z := by
  intro local_field Z
  unfold NN.State.gibbsUpdateSingleNeuron
  let probs : Bool → ENNReal := fun b =>
...
-/
@[simp]
lemma gibbs_update_single_neuron_prob
...
lemma uniform_neuron_selection_prob_valid : let p := λ (_ : U) => (1 : ENNReal) / (Fintype.card U : ENNReal)
