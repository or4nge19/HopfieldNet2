...
/-- Generic Hamming distance on functions over a finite index type. -/
def hammingDistanceOn {α ι : Type} [DecidableEq α] [Fintype ι]
    (x y : ι → α) : ℕ :=
  Fintype.card {i : ι // x i ≠ y i}
