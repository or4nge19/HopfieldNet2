...
/-- The paper's asynchronous one-step flow as a shared `ThreeD` deterministic dynamics object. -/
def deterministicDynamics [DecidableEq R] [DecidableEq U] (useq : ℕ → U) :
    DeterministicDynamics (Params (HopfieldNetwork R U)) (PhaseSpacePoint R U) where
  step wθ s := PhaseSpaceFlow wθ useq s
...
def BasinOfAttraction [DecidableEq R] [DecidableEq U]
  ...
  {s | ∃ n : ℕ, seqStates wθ s useq n = p ∧ FixedPoint wθ p}
...
/-- Real-valued Hopfield energies viewed through the shared `ThreeD` energy interface. -/
noncomputable def energyModel {U : Type} [DecidableEq U] [Fintype U] [Nonempty U]
    (wθ : Params (HopfieldNetwork ℝ U)) : EnergyModel (PhaseSpacePoint ℝ U) where
  energy := EnergyLandscape wθ
...
theorem convergence_to_fixed_point [DecidableEq R] [DecidableEq U]
      ...
      (useq : ℕ → U) (_hf : fair useq) : Set (PhaseSpacePoint R U) :=
